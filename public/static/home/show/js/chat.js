...
var liveType=
{
	...
	other:function()
	{
		...
		var stand="";
		var title=$("#title").val();
		var live_class_id=$("#live_class").val();
        ...
        var ctroom={"title":title,"type":type,"stand":stand,"stream":_DATA.anchor.stream,"uid":_DATA.user.id,"token": _DATA.user.token,"live_class_id":live_class_id};
	...
	type:function(){
		...
		//腾讯云
		if(_DATA.cdn_switch ==1 || _DATA.isvideo==1){
			var data_play={
				"autoplay": true
			};
...
var Controls={
      		...
      		success: function(data){
				...
				if(data.code!=0){
					...
					if(data.isadmin==1){
							...
							ct_en=data.user_nickname+" is set as administrator";
						}else{
							ct=data.user_nickname+"被删除管理员";
							ct_en=data.user_nickname+" was deleted administrator";
					}
					var msg='{"msg":[{"_method_":"SystemNot","action":"13","ct":"'+ct+'","ct_en":"'+ct_en+'","msgtype":"4","uname":"'+_DATA.user.user_nickname+'","toname":"'+data.user_nickname+'","touid":'+touid+',"uid":'+_DATA.user.id+',"level":'+_DATA.user.level+',"vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"}';
					Socket.emitData('broadcast',msg);
				}
 		...
 		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					...
					var msg = '{"msg":[{"_method_":"ShutUpUser","action":"1","ct":"'+name+'被禁言了","ct_en":"'+name+' has been banned","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nickname+'","touid":'+touid+',"toname":"'+name+'","vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"}';
					Socket.emitData('broadcast',msg);
				}else{
					if(data.code==700){
						layer.msg(data.msg,{},function(){
	...
	tiren:function()//踢人
	{
		...
		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					layer.msg("操作成功");
				 	var msg = '{"msg":[{"_method_":"KickUser","action":"2","ct":"'+name+'被踢出房间","ct_en":"'+name+' was kicked out of the room","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nickname+'","touid":'+touid+',"touname":"'+name+'","vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"} ';
					Socket.emitData('broadcast',msg); 
				}else{
					if(data.code==700){
						layer.msg(data.msg,{},function(){
	...
	stopRoom:function(type)//超管关闭直播0 禁止直播1
	{
		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					 var msg = '{"msg":[{"_method_":"stopLive","action":"10","ct":"该直播间涉嫌违规，已被停播","ct_en":"The live broadcast room was suspected of violating regulations and has been suspended.","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nickname+'","touid":'+_DATA.anchor.id+',"touname":"'+_DATA.anchor.id.user_nickname+'","vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"} ';
					Socket.emitData('broadcast',msg);
					layer.msg(data.msg);
				}else{
					if(data.code==700){
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					...
					var ct_en=_DATA.user.user_nickname+" followed the anchor";
					var msg='{"msg":[{"_method_":"SystemNot","action":"13","ct":"'+ct+'","ct_en":"'+ct_en+'","msgtype":"4","uname":"'+_DATA.user.user_nickname+'","toname":"'+_DATA.anchor.user_nickname+'","touid":'+_DATA.anchor.id+',"uid":'+_DATA.user.id+',"level":'+_DATA.user.level+',"vip_type":"0","liangname":"0","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}],"retcode":"000000","retmsg":"ok"}';
				Socket.emitData('broadcast',msg);
				}else{
					if(data.error==700){
						layer.msg(data.msg,{},function(){
...
var Light={
	...
	light:function()
	{
		...
		{
			 _DATA.user.light=1;
			var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendMsg","action":"0","msgtype":"2","level":"'+_DATA.user.level+'","uname":"'+_DATA.user.user_nickname+'","uid":"'+_DATA.user.id+'","heart":"1","ct":"我点亮了","ct_en":"I lit up","vip_type":"'+ _DATA.user.vip.type +'","liangname":"'+ _DATA.user.liang.name+'","usertype":"'+ _DATA.usertype+'","guard_type":"'+ _DATA.user.guard_type+'"}]}';
			Socket.emitData('broadcast',msg);
		}
		/* if(time<=timestamp)
		{	
