...
class PaidprogrampayController extends HomebaseController {
	...
	*/
	...
	public function logali($msg){
		file_put_contents(CMF_ROOT.'log/think/appapi/paidprogrampay/logali_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg.PHP_EOL.PHP_EOL,FILE_APPEND);
	}		
	/* 打印log */
	public function logwx($msg){
		file_put_contents(CMF_ROOT.'log/think/appapi/paidprogrampay/logwx_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg.PHP_EOL.PHP_EOL,FILE_APPEND);
	}			
