...
class PaypalbackController extends HomebaseController {
	...
	public function logpaypal_order($msg){
		file_put_contents(CMF_ROOT.'log/think/appapi/paypalback/logpaypal_order_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}

	/* 打印log */
	public function logpaypal_paidprogram($msg){
		file_put_contents(CMF_ROOT.'log/think/appapi/paypalback/logpaypal_paidprogram_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
