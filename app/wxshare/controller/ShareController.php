...
class ShareController extends HomebaseController {
	...
	public function show(){
		...
		$this->assign('chatserver',$chatserver);
		...
		$liveinfo=Db::name("live")
            ->field("uid,islive,stream,pull,isvideo,type,goodnum,live_type,pkuid,pkstream")
            ->where(['uid'=>$roomnum,'islive'=>1])
            ->find();
		...
		if(!$liveinfo){
			...
			$liveinfo['pull']='';
			$liveinfo['isvideo']=1;
			$liveinfo['stream']=$roomnum."_".time();
			$liveinfo['live_type']='0';
		}
        ...
        $liveinfo['user_nickname']=$anchor['user_nickname'];
		...
		if($liveinfo['isvideo']==1){
			...
			$hls='';
			//腾讯云
			if($configpri['cdn_switch']==1){
				$hls=PrivateKeyA('http',$stream.'.m3u8',0);
			}else{
				...
				//获取连麦用户的信息
				$linkmic_user=hGet('ShowVideo',$roomnum);
				...
				$hls=PrivateKeyA('http',$stream.'.flv',0);
		...
		if($uid){
			...
			->where(['uid'=>$uid,'touid'=>$liveinfo['uid']])
			->find();
