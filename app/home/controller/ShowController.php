  ...
  //首页
	public function index(){
		...
		$liveinfo=Db::name("live")
            ->where("uid='{$anchorinfo['id']}' and islive=1")
            ->find();
		...
		if($uid==$anchorinfo['id']){ //推流
			...
			if($liveban_info){
				...
				if($ban_endtime==0 && $ban_type=='all'){
					$this->assign('jumpUrl','/');
					$this->error("你已经被永久封禁直播,请联系管理员");
				}

				if($ban_endtime>0){
					if($ban_endtime<=$nowtime){
						Db::name('live_ban')->where(['liveuid'=>$uid])->delete();
					}else{
						$this->assign('jumpUrl','/');
						$this->error("你已经被封禁直播，".date("Y-m-d H:i:s",$ban_endtime)."到期");
					}
				}
			}
			...
			$this->assign('sw_push_length',$configpri['sw_push_length']);
			...
			//判断是否为语音聊天室
			if(isset($liveinfo['live_type']) && $liveinfo['live_type']==1){
				$this->assign('jumpUrl','/');
				$this->error('请下载APP观看语音聊天室');
			}

			if(isset($liveinfo['isvideo']) && $liveinfo['isvideo']==1){
				$isvideo=1;
			}
			...
			if($cdn_switch==2){
				$sw_rtc_token=getShengWangRtcToken($uid,$stream);
				if(isset($liveinfo['pkstream']) && $liveinfo['pkstream']){
					$linkmic_stream=$liveinfo['pkstream'];
					$sw_rtc_linkmic_token=getShengWangRtcToken($uid,$linkmic_stream);
				}

				//获取连麦用户的信息
				
				$linkmic_user=hGet('ShowVideo',$anchorid);
		...
		$this->assign('isvideo',$isvideo);

		//推拉流服务器类型
		$this->assign('cdn_switch',$cdn_switch);
		
		if(isset($liveinfo['anyway']) && $liveinfo['anyway']==1){
	...
	*/
	...
	public function setNodeInfo() {
		...
		if($uid>0){
			...
			/* 游客 */
			$sign= mt_rand(1000,9998);
			$info['id'] = '-'.$sign;
			$info['user_nickname'] = '游客'.$sign;
			$info['avatar'] = '';
			$info['avatar_thumb'] = '';
			$info['sex'] = '0';
			$info['signature'] = '0';
