		...
		if ($this->request->isPost()) {
            ...
            if($shop_status>0){
                ...
                if($shop_status==1){ //审核通过
                    $title='你的店铺审核已通过。';
                    $title_en='Your store review has been passed.';
                }else if($shop_status==2){ //审核失败
                    $title='你的店铺审核失败。';
                    $title_en='Your store review failed.';
                    if($reason){
                        $title.='失败原因：'.$reason;
                    }
                ...
                txMessageTpns('店铺审核',$title,'single',$uid,[],json_encode(['type'=>1]),'zh-cn');
                sleep(2);
                txMessageTpns('Store review',$title_en,'single',$uid,[],json_encode(['type'=>1]),'en');
