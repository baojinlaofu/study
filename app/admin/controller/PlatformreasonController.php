...
class PlatformreasonController extends AdminbaseController {
	...
	function addPost(){
		if ($this->request->isPost()) {
            ...
            if($name_en==""){
                $this->error("请填写英文退款原因");
            }
            
            $isexist=DB::name('shop_platform_reason')->where(['name_en'=>$name_en])->find();
            if($isexist){
	...
	function editPost(){
        if ($this->request->isPost()){
            ...
            $isexist=DB::name('shop_platform_reason')->where([['id','<>',$id],['name_en','=',$name_en]])->find();
            if($isexist){
                $this->error("英文退款原因已存在");
            }
