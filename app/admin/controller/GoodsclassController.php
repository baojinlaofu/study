...
class GoodsclassController extends AdminbaseController {
    ...
    function index(){
        ...
        $lists->each(function($v,$k){
            ...
            $two_list=Db::name("shop_goods_class")
                ->field("gc_id,gc_name,gc_name_en,gc_isshow,gc_sort,gc_icon,gc_addtime,gc_edittime")
                ->where("gc_parentid={$v['gc_id']}")
                ->order("gc_sort")
                ->select()
                ->toArray();
    ...
    */
    function addPost(){
        ...
        if(mb_strlen($gc_name)>30){
            $this->error("分类中文名称在30字符以内");
        }

        if(mb_strlen($gc_name_en)>30){
            $this->error("分类英文名称在30字符以内");
        }

        //判断分类名称是否重复
        $isexist=Db::name("shop_goods_class")->where(['gc_name'=>$gc_name])->find();
        if($isexist){
            $this->error("分类中文名称已经存在");
        }

        $isexist=Db::name("shop_goods_class")->where(['gc_name_en'=>$gc_name_en])->find();
        if($isexist){
            $this->error("分类英文名称已经存在");
        }
    ...
    function editPost(){
        ...
        if($gc_parentid==0){
            ...
            if($gc_grade==2){ //当前分类是二级分类
                ...
                Db::name("shop_goods_class")
                    ->where("gc_parentid={$gc_id}")
                    ->update(
                        array(
                            "gc_one_id"=>$gc_id,
                            "gc_grade"=>2,
   ...
   protected function getAdminClassList(){
        ...
        //获取一级分类
        $lists=Db::name("shop_goods_class")
            ->field("gc_id,gc_name,gc_name_en,gc_grade")
            ->where("gc_parentid=0")
            ->order("gc_sort")
            ->select()
            ->toArray();
    ...
    */
    protected function resetCache(){
        ...
        $lists=Db::name("shop_goods_class")
            ...
            ->where("gc_parentid=0 and gc_isshow=1")
            ->order("gc_sort")
            ->select()
            ->toArray();
        ...
        foreach ($lists as $k => $v) {
            ...
            $two_list=Db::name("shop_goods_class")
                ->field("gc_id,gc_name,gc_name_en")
                ->where("gc_parentid={$v['gc_id']} and gc_isshow=1")
                ->order("gc_sort")
                ->select()
                ->toArray();
            ...
            if($two_list){
                foreach ($two_list as $k1 => $v1) {
                    ...
                    $three_list=Db::name("shop_goods_class")
                        ->field("gc_id,gc_name,gc_name_en")
                        ->where("gc_parentid={$v1['gc_id']} and gc_isshow=1")
                        ->order("gc_sort")
                        ->select()
                        ->toArray();
    ...
    */
    public function resetOneCache(){
        ...
        $list1=Db::name("shop_goods_class")
            ->field("gc_one_id")
            ->where("gc_grade=3")
            ->select()
            ->toArray();
        ...
        $list=Db::name("shop_goods_class")
            ->field("gc_id,gc_name,gc_name_en,gc_isshow")
            ->where($map)
            ->where('gc_id','in',$ids)
            ->order("gc_sort")
            ->select()
