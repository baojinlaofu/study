...
class Home extends Api {
	public function getRules() {
		return array(
            ...
            'getVoiceLiveList'=>array(
                'p' => array('name' => 'p', 'type' => 'int', 'default'=>'1' ,'desc' => '页数'),
                'mic_count' => array('name' => 'mic_count', 'type' => 'int', 'default'=>'9' ,'desc' => '连麦人数3-5-9'),
            ),
    ...
    public function getVoiceLiveList(){
        ...
        $p=\App\checkNull($this->p);
        $mic_count=\App\checkNull($this->mic_count);
        $domain = new Domain_Home();
       
        $key2="getVoiceLiveList_".$p.'_'.$mic_count;
        $list=\App\getcaches($key2);
        if(!$list){
            $list = $domain->getVoiceLiveList($p,$mic_count);
            \App\setcaches($key2,$list,2); 
        }
